...
<article>
	<h1>상품수정</h1>
	<form name="frm" method="post" enctype="mulipart/form-data">
		<input type="hidden" name="pseq" value="${productVO.pseq }">
		<input type="hidden" name="code">
		<input type="hidden" name="nonmakelmg" value="${productVO.image }">
				...
				<select name="kind">
					...
					<c:choose>
						<c:when test="${productVO.kind==status.count }">
							<option value="${status.count}" selectecd="selected">${kind}</option>
						</c:when>
